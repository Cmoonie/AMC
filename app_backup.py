...
with knop_col:
    ...
    zoek_geklikt = st.button("🔍 Zoeken", type="primary", use_container_width=True)
...
if zoekterm:
    ...
    # ==========================================
    # Combineer normale zoekresultaten
    # ==========================================
    resultaat = pd.concat(
    ...
    if top_pmids:
        ...
        # Department-filter ook hier toepassen
        if department_filter != "Alle":
    ...
    with col_links:
        ...
        for _, persoon in resultaat.iterrows():
            ...
            actief = persoon["id"] in actieve_leiders
            status = "🟢 Lopend project" if actief else "Onderzoeker"

            with st.container(border=True):

                st.markdown(f"### 👤 {persoon['name']}")
                st.caption(status)

        # Expertise van deze onderzoeker
        exp_ids = personen_expertise[
            personen_expertise["person_id"] == persoon["id"]
        ]["expertise_id"].tolist()
        ...
        # Knoppen onderaan de card
        col1, col2 = st.columns([2, 1])

        with col1:
            if st.button(
                "Bekijk profiel",
                key=f"persoon_{persoon['id']}",
                type="primary",
            ...
            if actief:
                if st.button(
                    ...
                    if len(projecten_persoon) == 1:
                        ...
                        st.switch_page(
                            "pages/lopende_projecten_overzicht.py"
                        )
